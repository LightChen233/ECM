...
def run(total=1,
        ...
        temperature=0.1,
        shot=5):
    test_data_path = f"semantic/bert_embed/{data_name}_test.jsonl"
    save_dir = f"experiments/icl-explanation/different_embedder/request_data/{data_name}-bert"
    if data_name == "biggsm":
    ...
    asyncio.run(request_LLM(
        ...
        save_path=fr"{save_dir}/{model_name}-{shot}-tp-08.jsonl",
        consumer_size=25,
